import numpy as np
import torch
import h5py

from style_picker import StylesPicker
from wct2_style_transfer.utils.io import open_image, save_image
from wct2_AdaIn import WCT2
encoder_path = ''
decoder_path = ''

input_images_path = ''
style_lib_path = ''
transfer_at = {'encoder', 'decoder', 'skip'}
option_unpool = 'cat5'


def load_images(path):
    with h5py.File(path, 'r') as hf:
        return np.array(hf.get('LDCT'))


def style_transfer(wct_loaded_model, content_image_path, style_image_path, image_size=512):
    content_image = open_image(content_image_path, image_size).to(device)
    style_image = open_image(style_image_path, image_size).to(device)
    with torch.no_grad():
        styled_image = wct_loaded_model.transfer(content_image, style_image, alpha=1)
    save_image(styled_image.clamp_(0, 1), 'styled' + content_image_path, padding=0)
    # numpy_img = styled_image.clamp_(0, 1).cpu()[0, :, :, :].permute(1, 2, 0).numpy()
    # plt.imshow(numpy_img)
    # for i in range(3):
    #     plt.imsave(str(i) + 'styled' + content_image_path, numpy_img[:, :, i], cmap='gray')


device = 'cpu' if not torch.cuda.is_available() else 'cuda:0'
device = torch.device(device)

wct_model = WCT2(transfer_at=transfer_at, option_unpool=option_unpool, device=device)
input_images = load_images(input_images_path)
style_picker = StylesPicker('/Users/evgenia/Desktop/диплом/cnn-vgg/style_data.hdf5')

#  здесь будет: цикл по всем входным изображениям
#  для каждого находим стиль с помощью StylesPicker
#  делаем style transfer и сохраняем обратно в input_images_path
